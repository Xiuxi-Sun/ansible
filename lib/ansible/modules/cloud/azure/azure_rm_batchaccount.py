...
class AzureRMBatchAccount(AzureRMModuleBaseExt):
    ...
    def __init__(self):
        self.module_arg_spec = dict(
            ...
            location=dict(type='str', updatable=False, disposition='/'),
            auto_storage_account=dict(type='raw'),
            key_vault=dict(type='raw', updatable=False, disposition='/'),
            pool_allocation_mode=dict(default='batch_service', type='str', choices=['batch_service', 'user_subscription'], updatable=False, disposition='/'),
    ...
    def create_update_batchaccount(self):
        ...
        try:
            if self.to_do == Actions.Create:
                response = self.mgmt_client.batch_account.create(resource_group_name=self.resource_group, account_name=self.name, parameters=self.batch_account)
            else:
                response = self.mgmt_client.batch_account.update(
                    resource_group_name=self.resource_group, account_name=self.name, tags=self.tags, auto_storage=self.batch_account.get('auto_storage')
                )
    ...
    def delete_batchaccount(self):
        ...
        try:
            response = self.mgmt_client.batch_account.delete(resource_group_name=self.resource_group, account_name=self.name)
